...
async function getTransactionFilteredBy2(id, type) {
  const list = await sql`SELECT * 
FROM transaction
LEFT JOIN categories 
  ON transaction.categoryId = categories.id 
WHERE transaction.categoryId = ${id} and transaction.type = ${type}; `;
  console.log({ list })
  return list;
}
...
module.exports = {
  ...
  updateCategories, deleteCategories, getTransaction, createNewTransaction, updateTransaction, deleteTransaction, getTransactionFiltered, getTransactionFilteredByType, getTransactionFilteredBy2
}
